...
public class LlmProviderRegistry {
     ...
     * 获取视觉理解专用 ChatClient。
     *
     * <p>视觉模型与普通文本模型共享同一 Provider 的 baseUrl 和 API Key，
     * 但使用独立模型名和缓存项，避免污染默认文本模型配置。</p>
     */
    public ChatClient getVisionChatClient(String providerId, String visionModel) {
        String id = resolveProviderId(providerId);
        if (isBlank(visionModel)) {
            throw new BusinessException(ErrorCode.PROVIDER_CONFIG_READ_FAILED,
                "未配置视觉模型，无法解析图片简历");
        }
        String normalizedModel = visionModel.trim();
        return clientCache.computeIfAbsent(
            id + ":vision:" + normalizedModel,
            key -> createVisionChatClient(id, normalizedModel)
        );
    }
    ...
    private ChatClient createVisionChatClient(String providerId, String visionModel) {
        OpenAiChatModel chatModel = buildChatModel(providerId, visionModel);
        log.info("[LlmProviderRegistry] Created vision ChatClient for provider={}, model={}",
            providerId, visionModel);
        // 图片文字提取不挂载 Skills、Memory 或 Tool Advisor，避免无关上下文和工具调用干扰。
    ...
    private OpenAiChatModel buildChatModel(String providerId) {
        return buildChatModel(providerId, null);
    }

    private OpenAiChatModel buildChatModel(String providerId, String modelOverride) {
        ProviderSnapshot config = loadProviderOrThrow(providerId);
        String model = isBlank(modelOverride) ? config.model() : modelOverride.trim();
        log.info("[LlmProviderRegistry] Building ChatModel - Provider: {}, BaseUrl: {}, Model: {}",
                 providerId, config.baseUrl(), model);
